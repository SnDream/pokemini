...
PokeMini+ is a fork of Gameblabla's PokeMini, aimed at a better experience on the RS-90.

This fork tries to fix the menu UI, add new 2.5x scaling, and move EEPROM to internal storage.

> Scaling:

  2.5x (No Filter):
  Original 1x rendering, 2.5x scaling output
  No LCD Filter support

  2x:
  Original 2x rendering, direct output

> V-Sync:

  On:
  Reduces video tearing (especially when rumbling)
  May cause slowdown or frameskip

  Off:
  Smoother emulation speed
  May cause video tearing

> Custom Bios:

  Place "bios.bin" on "~/.pokemini"
